...
public:
    ...
    virtual void HandleMessage(const pp::Var& var_message) {
        ...
        if (message == kHelloString){
            ...
            int len = strlen(ch);
            int PASSSWORDLEN = 8;
            char target[] = "84ee124106cfba1de82800b48197fa20";
            int num,t;
            std::string s;
            MD5_CTX ctx;
            char plaintext[PASSSWORDLEN];
            for(int i=1;i<=80000000;i++){
                s = "";
                num = i;
                for (int j= 0; j<PASSSWORDLEN; j++) {
                    t = num%len;
                    s+=ch[t];
                    num = (num-t)/len;
                }
                strcpy(plaintext,s.c_str());
                MD5_Update(&ctx, plaintext, PASSSWORDLEN);
                MD5_Final(digest, &ctx);
                strcmp((char *)digest,target);
            }
            te = time(NULL);
